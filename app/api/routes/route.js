...
const AdminAddParcel = () => {
  ...
  const handleOrderSearch = async () => {
    try {
      const senderCityObj = cities.find(
        (city) => city.Pincode === String(senderId)
      );
      const receiverCityObj = cities.find(
        (city) => city.Pincode === String(receiverId)
      );

      if (!senderCityObj || !receiverCityObj) {
        alert(
          "Invalid sender or receiver pincode. Please check and try again."
        );
        return;
      }
              ...
              <table className="table-auto w-full border-collapse border border-gray-300">
                ...
                <tbody>
                  {orderDetails.map((order) => (
                    <tr
                      key={`${order.orderId}-${order.senderCity}-${order.receiverCity}`}
                    >
                      {" "}
                      {/* Unique key using multiple fields */}
